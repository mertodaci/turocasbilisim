...
function migrateLegacyJobTrackingRename() {
  ...
  // İş Takibi Tanımlar ekranı ikiye bölündü (Bilet Durumları + Bilet
  // Tipleri) — eski `is_takibi_tanimlar` yetkisi her iki yeni anahtara da
  // kopyalanır (erişim kaybı olmasın); eski anahtar aşağıdaki genel temizlik
  // bloğunda silinir.
  try {
    const { v4: uuidv4 } = require('uuid');
    const now = new Date().toISOString();
    const oldPerms = db.prepare("SELECT role_name, can_view, can_add, can_edit, can_delete FROM role_permissions WHERE module='is_takibi_tanimlar'").all();
    for (const p of oldPerms) {
      for (const newModule of ['is_takibi_bilet_durumlari', 'is_takibi_bilet_tipleri']) {
...
function initDb() {
  ...
  // "yardim" modülü (Destek Merkezi altındaki yeni menü öğesi) için:
  // support_center'ı görebilen (can_view=1) her rol yardim'ı da görsün —
  // yalnızca yardim satırı hâlâ can_view=0 iken (admin bilerek kapattıysa
  // ezilmesin diye), idempotent.
  try {
    const supportViewers = db.prepare("SELECT role_name FROM role_permissions WHERE module='support_center' AND can_view=1").all();
    supportViewers.forEach(({ role_name }) => {
      db.prepare("UPDATE role_permissions SET can_view=1 WHERE role_name=? AND module='yardim' AND can_view=0").run(role_name);
    });
  } catch (e) { console.error('yardim izin senkronu:', e.message); }
  ...
  const moduleGuidesExistedBefore = db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='module_guides'").get();
  try {
    db.exec(`
  ...
  // module_guides — yalnızca tablo bu boot'tan ÖNCE yoksa (ilk kurulum) seed
  // edilir; admin'in sildiği bir kılavuz bir daha ASLA geri gelmesin diye.
  if (!moduleGuidesExistedBefore) {
    try {
      const seedGuide = db.prepare(`INSERT OR IGNORE INTO module_guides (id, module_key, summary, steps) VALUES (?, ?, ?, ?)`);
      const { v4: uuidv4Guide } = require('uuid');
      const GUIDE_SEED = [
        ["dashboard", "Uygulamaya girince karşılanan ana ekran. Bekleyen onaylar, bugünkü izinli/doğum günü olan personel, duyurular, sözleşme/stok/bilet uyarıları ve son 7 günün bilet hareketi tek bakışta özetlenir. Widget'lar sürükle-bırak ile yeniden düzenlenebilir, istenmeyenler kapatılıp \"Gizli Widget'lar\"dan geri eklenebilir.", [
        ...
        ["insan_kaynaklari", "Çalışan kayıtlarının, organizasyon şemasının, özlük evraklarının, izin/harcama taleplerinin ve zam işlemlerinin yönetildiği modül. Genel Tanımlar (departman/pozisyon/izin türleri), Çalışanlar listesi ve raporlar buradan yönetilir.", [
        ...
        ["pdks_vardiya", "Kart/PDKS geçişlerinin (giriş-çıkış saatleri) izlendiği ve vardiya atamalarının yapıldığı modül. Kart Yönetimi ham geçiş kayıtlarını, Vardiya Transferi personel vardiya değişimlerini yönetir.", [
        ...
        ["devriye_yonetimi", "Güvenlik devriye/checkpoint takibi: lokasyon ve QR noktalarının tanımlandığı, güvenlik personelinin vardiyaya atandığı ve sahada QR okutarak devriye kaydı oluşturduğu modül.", [
        ...
        ["maas_bordro", "Aylık bordro hesaplamasının uçtan uca yapıldığı modül: puantaj üretiminden, mesai onayına, kesintilere, bordro hesaplama/onaylamaya ve ay kapanışına kadar sıralı bir iş akışı izler. Gelir vergisi kümülatif hesaplandığı için aylar SIRAYLA işlenmelidir.", [
        ...
        ["stok_yonetimi", "Ürün/depo/raf tanımlarından stok giriş-çıkış-transfer fişlerine, FIFO parti takibine, sayım ve zimmete kadar tüm depo operasyonlarının yönetildiği kapsamlı bir modül.", [
        ...
        ["is_takibi", "Müşteri taleplerinin (bilet) açılıp takip edildiği, proje/kanban panolarıyla iş akışının yönetildiği destek/proje takip modülü. Bilet durumları ve tipleri kendi tanım ekranından özelleştirilebilir.", [
        ...
        ["musteriler_menu", "Müşteri (cari) kayıtlarının, müşteri portalı kullanıcılarının ve müşteriye özel tanımların (müşteri tipi, şehir vb.) yönetildiği modül.", [
        ...
        ["sozlesme_yonetimi", "Müşterilerle yapılan sözleşmelerin ve bu sözleşmelere bağlı hakediş (ödeme planı) takibinin yapıldığı modül. Sözleşme türü/ürün/modül gibi seçim listeleri kendi tanım ekranından yönetilir.", [
        ...
        ["ebys", "Kurum içi/dışı yazışmaların (gelen-giden evrak) kayıt altına alındığı elektronik belge yönetim modülü. KEP (Kayıtlı Elektronik Posta) ve e-imza altyapısı hazırlık aşamasında — gerçek sağlayıcı bağlantısı henüz yok.", [
        ...
        ["arsiv_yonetimi", "Silinmemiş ama artık aktif kullanılmayan/kapanmış kayıtların (süresi dolan sözleşmeler, ayrılmış personel evrakları, kapanmış bilet ve bordro dönemleri, müşteri evrak arşivi) tek yerden görüntülendiği, salt-okunur bir modül.", [
        ...
        ["system_admin", "Uygulamanın idari yönetim ekranları: kullanıcı hesapları, rol bazlı yetkilendirme, oturum yönetimi, duyurular, çöp kutusu, denetim kaydı ve sunucu sağlığı. Çoğu ekran yalnızca admin/yönetici rolüne açıktır.", [
        ...
        ["support_center", "Kullanıcıların günlük iş akışını destekleyen kişisel araçlar: iç mesajlaşma, yapılacaklar listesi, harcama talepleri, izin talepleri ve kişisel takvim.", [
      ...
      for (const [moduleKey, summary, steps] of GUIDE_SEED) {
        seedGuide.run(uuidv4Guide(), moduleKey, summary, JSON.stringify(steps));
      }
    } catch (e) { console.error('module_guides seed:', e.message); }
  }
