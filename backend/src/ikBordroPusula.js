// Ücret Pusulası HTML üretimi — sunucu tarafı (dosya olarak diske yazılıp
// arşivlenir). src/lib/ikBordroPusula.js'teki tarayıcı sürümüyle aynı HTML
// gövdesi; window.open/document.write/window.print çağrıları burada yok
// (DOM'a bağımlı değil, saf string üretimi).
const esc = (v) => String(v ?? "").replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]));
const nf = (v) => (Number(v) || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });

function pusulaHtmlUret(satir, donem, sirket) {
  const firmaAdi = sirket?.unvan || "Turkonix — Sınırsız İletişim";
  const r = (l, v) => `<tr><td>${esc(l)}</td><td class="r">${nf(v)} ₺</td></tr>`;
  return `<!doctype html><html lang="tr"><head><meta charset="utf-8"><title>Ücret Pusulası — ${esc(satir.personel_adi)}</title>
<style>
  ...
  <div class="head">
    <div><div class="firma">${esc(firmaAdi)}</div><div style="color:#555">İnsan Kaynakları · Ücret Pusulası</div></div>
