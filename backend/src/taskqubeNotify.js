...
function resolveCustomerEmail(ticket) {
  ...
  // bileti actigi otomatik atama (TQTicketFormDialog.jsx) hem de analistin
  // "Musteri Muhatabi" dropdown'u (TQTicketDetailDialog.jsx) flowApi.auth.users()
...
// TEST MODU: TASKQUBE_MAIL_TEST_TICKET .env'de tanimliysa, mail SADECE o
...
function isTestModeBlocked(ticketNumber) {
  const testTicket = process.env.TASKQUBE_MAIL_TEST_TICKET;
...
function notifyTicketUpdate(existing, updated, actorUser) {
  ...
  if (isTestModeBlocked(num)) {
    console.log(`[taskqubeNotify] test modu aktif (TASKQUBE_MAIL_TEST_TICKET=${process.env.TASKQUBE_MAIL_TEST_TICKET}), bilet #${num} atlandi`);
    return;
  }
  ...
  if (customerLabel) {
    ...
    if (email) {
      ...
      console.warn(`[taskqubeNotify] bilet #${num} için müşteri e-postası bulunamadı (customer_contact_id/customer_id eksik veya kayıtlı kişide email yok)`);
    }
  ...
  let actionLines;
  if (statusChanged) {
    const statusRow = db.prepare('SELECT name FROM tq_ticket_statuses WHERE key = ? LIMIT 1').get(updated.status);
    actionLines = [
