...
# ---------- 1) Backend: expenseTrend'e filtre + bekleyen sorgusu ----------
patch("backend/src/index.js",
    ...
    `).all();""",
    "expenseTrendPending",
    "backend trend sorgulari")

# ---------- 2) Backend: expenseByCategory filtresi ----------
patch("backend/src/index.js",
    """    const expenseByCategory = db.prepare(`
      SELECT
        COALESCE(SUM(accommodation),0) as accommodation,
        COALESCE(SUM(transport),0) as transport,
        COALESCE(SUM(fuel),0) as fuel,
        COALESCE(SUM(meal),0) as meal,
