...
if "is_deleted" not in cols:
    ...
    con.commit()
    print("TAMAM: sales_activities.is_deleted kolonu eklendi")
else:
...
# ---------- 3) entityRouter SOFT_DELETE_TABLES ----------
...
# ---------- 8) OffersPage: onay dialogu ----------
old_tail = """        </table>"""
p = os.path.join(ROOT, "src/pages/OffersPage.jsx")
with open(p, encoding="utf-8") as f:
    s = f.read()
if "AlertDialogContent" in s and "toDelete" in s and "alertdialog-eklendi" not in s:
    assert s.count(old_tail) == 1, "ANCHOR BULUNAMADI/COKLU: OffersPage table kapanisi"
    yeni = old_tail + """
      {/* alertdialog-eklendi */}
    ...
    s = s.replace(old_tail, yeni)
    with open(p, "w", encoding="utf-8") as f:
        f.write(s)
    print("TAMAM: OffersPage onay dialogu")
else:
