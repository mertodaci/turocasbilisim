...
import JTTicketDetailDialog from "./JTTicketDetailDialog";
...
export default function JTKanbanBoard({ tickets, employees = [], projects = [], customers = [], boardId = null, boardName = null }) {
  ...
  const canDeleteList = can(_role, "is_takibi_kanban", "delete");
  const canEditList = can(_role, "is_takibi_kanban", "edit");
  ...
  const deleteStatusMutation = useMutation({
    mutationFn: (id) => flowApi.entities.JTTicketStatus.update(id, { is_active: false }),
    onSuccess: () => {
  ...
  const renameStatusMutation = useMutation({
    mutationFn: ({ id, name }) => flowApi.entities.JTTicketStatus.update(id, { name }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tq-statuses"] });
  ...
  const reorderStatusMutation = useMutation({
    mutationFn: ({ id, sort_order }) => flowApi.entities.JTTicketStatus.update(id, { sort_order }),
  ...
  const archiveTicketsMutation = useMutation({
    mutationFn: async (ticketIds) => {
      await Promise.all(ticketIds.map(id => flowApi.entities.JTTicket.update(id, { status: "arsivlendi" })));
    },
    onSuccess: () => {
  ...
  const handleDragEnd = (result) => {
    ...
    // Kalici yazma
    Promise.all(writes.map((w) => flowApi.entities.JTTicket.update(w.id, w.data)))
  ...
  if (statuses.length === 0) {
    return (
      <div className="text-center py-12 text-muted-foreground text-sm">
        Bilet durumları tanımlanmamış. İş Takibi Tanımlar ekranından durum ekleyin.
      </div>
    );
  }
