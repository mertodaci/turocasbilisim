...
// İş Akışı sekmesi -- faz (grup) sırası ve etiketleri (diğer İş Takibi ekranlarıyla tutarlı)
...
export default function JTTicketDetailDialog({ ticket, employees, projects, customers = [], open, onOpenChange, isMusteri = false, canEdit = true }) {
  ...
  const linkRelatedMutation = useMutation({
    mutationFn: async ({ parentId, relatedIds }) => {
      ...
      for (const id of relatedIds) {
        if (id !== parentId) {
          await flowApi.entities.JTTicket.update(id, { parent_ticket_id: parentId });
        }
      }
      // Ana biletin kendi parent'ını temizle
      await flowApi.entities.JTTicket.update(parentId, { parent_ticket_id: "" });
    },
    onSuccess: () => {
  ...
  const editCommentMutation = useMutation({
    mutationFn: ({ id, content }) => flowApi.entities.JTComment.update(id, { content }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tq-comments", ticket?.id] });
  ...
  const deleteCommentMutation = useMutation({
    mutationFn: (id) => flowApi.entities.JTComment.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tq-comments", ticket?.id] });
  ...
  const updateTicketMutation = useMutation({
    mutationFn: ({ data }) => flowApi.entities.JTTicket.update(ticket.id, data),
