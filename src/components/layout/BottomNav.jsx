...
export default function BottomNav() {
  ...
  const [flyout, setFlyout] = useState(null); // { key, left, bottom } (alt/mobil) veya { key, left, top } (sol/masaüstü)
  const flyoutTimer = useRef(null);

  // sm breakpoint (640px) ve üzeri: menü sol dikey çubuğa döner (bu ekranın
  // aslen geldiği yer — bkz. yukarıdaki not), altındaki dar ekranlarda
  // mevcut yatay alt çubuk aynen kalır (kullanıcı isteği: tasarımı bozacaksa
  // mobilde değiştirme).
  ...
  return (
    <>
      <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 bg-card border border-border shadow-xl rounded-2xl px-2 py-1.5 flex items-center gap-1 overflow-x-auto max-w-[95vw] sm:bottom-auto sm:left-4 sm:top-1/2 sm:-translate-y-1/2 sm:translate-x-0 sm:flex-col sm:items-stretch sm:gap-1 sm:px-1.5 sm:py-2 sm:max-w-none sm:max-h-[85vh] sm:overflow-y-auto sm:overflow-x-visible">
        {navItems.map((item) => {
