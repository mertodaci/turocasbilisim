...
import { Plus, CheckCircle, XCircle, Clock, Calendar, User, Pencil, Trash2 } from "lucide-react";
...
export default function LeaveRequests() {
  ...
  const isPrivileged = user?.role === "admin" || user?.role === "yonetici" || can(user?.role, "leave_requests", "edit") || can(user?.role, "leave_requests", "add");

  const { data: leaves = [], isLoading } = useQuery({
    queryKey: ["leave-requests"],
    queryFn: () => flowApi.entities.LeaveRequest.list("-created_date", 200),
  });
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {isPrivileged ? "Tüm çalışan izin taleplerini görüntüleyin ve yönetin." : "İzin taleplerinizi oluşturun ve takip edin."}
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 border-b">
              <tr>
                {isPrivileged && <th className="text-left px-4 py-3 font-semibold text-muted-foreground">Çalışan</th>}
                ...
                {isPrivileged && <th className="text-left px-4 py-3 font-semibold text-muted-foreground">Onaylayan</th>}
            ...
            <tbody>
              {filtered.map((leave, i) => {
                ...
                const isOwn = leave.employee_email === user?.email || leave.created_by_id === user?.id;
                const approverName = leave.status === "onaylandi" ? leave.manager_approver_name : leave.ik_approver_name;
                return (
                  <tr key={leave.id} className={`border-b last:border-0 hover:bg-muted/20 transition-colors ${i % 2 === 0 ? "" : "bg-muted/10"}`}>
                    ...
                    {isPrivileged && (
                      <td className="px-4 py-3 text-xs">
                        {approverName ? (
                          <span className="text-muted-foreground">{approverName}</span>
                        ) : (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        {isPrivileged && (leave.status === "ik_onayi_bekliyor" || leave.status === "yonetici_onayi_bekliyor") && (
                          <Button size="sm" variant="outline" onClick={() => setSelectedLeave(leave)}>
                            İncele
                          </Button>
                        )}
                        {((isOwn && ['beklemede','ik_onayi_bekliyor','yonetici_onayi_bekliyor'].includes(leave.status)) || (isPrivileged && !['onaylandi'].includes(leave.status))) && (
                          <>
                            <Button
                              size="sm"
                              variant="outline"
                        ...
                        {leave.ik_approval_note && (
                          <span className="text-xs text-muted-foreground italic truncate max-w-[120px]" title={leave.ik_approval_note}>
                            "{leave.ik_approval_note}"
                          </span>
                        )}
                      </div>
