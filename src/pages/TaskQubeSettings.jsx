...
export default function TaskQubeSettings() {
  const queryClient = useQueryClient();
  ...
  const { data: statuses = [], isLoading } = useQuery({
    ...
    queryFn: () => flowApi.entities.TQTicketStatus.list("sort_order", 500),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data) => flowApi.entities.TQTicketStatus.create(data),
    onSuccess: () => { refreshStatuses(); toast.success("Durum eklendi"); },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => flowApi.entities.TQTicketStatus.update(id, data),
    onSuccess: () => { refreshStatuses(); setEditingStatusId(null); },
  });

  const bulkUpdateMutation = useMutation({
    mutationFn: ({ ids, data }) => Promise.all(ids.map((id) => flowApi.entities.TQTicketStatus.update(id, data))),
    onSuccess: () => { refreshStatuses(); setEditingStatusId(null); },
  });

  const deleteMutation = useMutation({
    mutationFn: (ids) => Promise.all((Array.isArray(ids) ? ids : [ids]).map((id) => flowApi.entities.TQTicketStatus.delete(id))),
  ...
  const handleSeedDefaults = async () => {
    for (const s of DEFAULT_STATUSES) {
      await flowApi.entities.TQTicketStatus.create(s);
    }
    refreshStatuses();
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <Settings className="w-6 h-6" /> TaskQube Tanimlar
          </h1>
