...
export default function UserDashboard() {
  ...
  // Kişisel "aktivite" kayıtları yalnız satış rolünde tutulur (sales_activities tablosu).
  const { data: activities = [] } = useQuery({ queryKey: ["user-sales-activities", employeeRecord?.id], queryFn: () => flowApi.entities.SalesActivity.filter({ employee_id: employeeRecord?.id }), enabled: isSatis && !!employeeRecord?.id });
  ...
  // Aktivite tipi dağılımı (bu hafta) — satış rolü
  ...
  const kpis = [
    ...(isSatis ? [{ to:"/satis-raporlari", val:todayActs.length, label:"Bugünkü Aktivite", sub:`${Math.floor(todayMin/60)}sa ${todayMin%60}dk`, grad:"from-blue-500 to-indigo-600", icon:Activity }] : []),
  ...
  return (
    <div className="space-y-5 max-w-7xl">
      ...
      <div className="flex items-center justify-between">
        ...
        {isSatis && (
          <Link to="/satis-aktivite-ekle?add=1"
      ...
      {/* HAFTALIK ÖZET */}
      <div className={cn("grid grid-cols-1 gap-5", isSatis ? "lg:grid-cols-3" : "lg:grid-cols-2")}>
        {isSatis && (
          <>
            {/* Aktivite Dağılımı */}
            <div className="bg-card rounded-2xl border border-border/50 shadow-sm p-5">
              ...
              {actChartData.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-40 text-muted-foreground">
                  ...
                  <Link to="/satis-aktivite-ekle?add=1" className="mt-2 text-xs text-indigo-500 hover:underline">Aktivite ekle →</Link>
                </div>
              ) : (
            ...
            <div className="bg-card rounded-2xl border border-border/50 shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link to="/satis-raporlari" className="text-xs text-indigo-500 flex items-center gap-1">Tümü<ArrowUpRight className="w-3 h-3"/></Link>
              </div>
              {recentActs.length === 0 ? (
                    ...
                    return (
                      <div key={a.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-muted/40 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {outcome&&<p className="text-[10px] text-muted-foreground truncate">{outcome}</p>}
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-xs font-bold">{formatDuration(a.duration_minutes)}</p>
                          <p className="text-[10px] text-muted-foreground">{a.date && format(new Date(a.date),"d MMM",{locale:tr})}</p>
                        </div>
                      </div>
                    );
                  })}
